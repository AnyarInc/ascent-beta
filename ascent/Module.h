...
namespace asc
{
   /** Removes const qualification from type, which allows Ascent to internally change Modules of type T that are const qualified: asc::Link<const T> */
   template <typename T>
   ...
   */
   ...
   private:
      ...
      // For file streaming or stringstream.
      template <typename T>
   ...
   */
   ...
   /** Generates an input file based on named Modules and associated variables.
   * @param file_name  The name of the file to be generated. Automatically appended with .asc
   */
   inline void generateInputFile(const std::string& file_name);
}